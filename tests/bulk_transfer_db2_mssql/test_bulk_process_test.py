...
class TestBulkProcessDB2MSSQLExtended:
    ...
    def test_Start_process_tc010(self, page):
        ...
        ProcessListPage(page).validate_process_part_input(raw)
        ProcessListPage(page).validate_process_part_input_reslute(raw)
        ProcessListPage(page).validate_process_part_ProcessingApplying(raw)
        ProcessListPage(page).validate_process_part_ProcessingApplying_reslute(raw)
